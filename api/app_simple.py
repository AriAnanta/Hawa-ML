...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Atau sesuaikan dengan origin frontend Anda
...
def preprocess_history(items: List[HistoryItem]) -> pd.DataFrame:
    ...
    if len(df_resampled) < MAX_LOOKBACK:
        raise HTTPException(
            status_code=400, 
            detail=f"Data tidak mencukupi. Tersedia {len(df_resampled)} jam setelah resample, butuh minimal {MAX_LOOKBACK} jam."
        )
...
if __name__ == "__main__":
    import uvicorn
    # Menggunakan string import "app_simple:app" agar fitur reload berfungsi di Windows
